...
const ContactDialogContext = React.createContext<ContactDialogApi | null>(null);
...
export function ContactDialogProvider({
  ...
  const api = React.useMemo<ContactDialogApi>(
    () => ({
      isOpen,
      open: () => setIsOpen(true),
  ...
  return (
    <ContactDialogContext.Provider value={api}>
      {children}
